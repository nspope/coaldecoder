...
# variable migration over time, and missing trees
#-------------------------------------------------------------------------#
...
parser.add_argument('--out', type=str, help='Path to output tree sequence (unmasked)')
...
tree_left = np.array([tree.interval.left for tree in ts.trees()])
tree_span = np.array([tree.interval.right - tree.interval.left for tree in ts.trees()])
mask_intervals = np.column_stack([tree_left, tree_left + 0.5*tree_span])
ts = ts.delete_intervals(mask_intervals)
n_masked = ts.num_trees

assert n_masked == n_unmasked * 2

print("Trees after masking: " + str(ts.num_trees)) #should be 2x original
handle = open(args.out + ".mask", "wb")
ts.dump(handle)
handle.close()
