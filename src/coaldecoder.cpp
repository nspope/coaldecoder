...
struct tally_tmrca
{
  ...
  tally_tmrca (const arma::cube& tmrca, const arma::umat& clades, const arma::uvec& populations, const arma::vec& breakpoints)
  {
    ...
    transition_rate_matrix trans_mat (num_pop, num_hap);
...
struct coalescent_epoch_gaussian
{
  transition_rate_matrix* rate_matrix;

  unsigned P, I, num_trans, num_coal, num_start, num_emiss;
  arma::umat c_map_e, s_mask_e;
  arma::uvec remap;
  arma::mat migr_mat, tran_probs, coal_probs, emiss_probs, y;
  double duration, loglikelihood;

  matrix_exponential_multiply transition;

  coalescent_epoch_gaussian (
      arma::mat& states, 
      arma::mat& y_in, 
      arma::uvec remap_in, 
      arma::mat migr_mat_in, 
      double duration_in, 
      transition_rate_matrix* rate_matrix
  ) : rate_matrix (rate_matrix)
    , P (rate_matrix->P) 
    ...
    , transition (arma::trans(rate_matrix->transition_rates(remap, migr_mat)), states, duration)
  {
    // update states (probabilities that uncoalesced lineages are in XXX configuration) and
    // calculate residual 

    if(!(states.n_rows == num_trans + num_coal && states.n_cols == num_start)) Rcpp::stop("invalid states");
    if(!(y.n_rows == num_emiss && y.n_cols == num_start)) Rcpp::stop("invalid y");
    ...
    for (unsigned i=0; i<num_start; ++i)
    {
      ...
      for (unsigned j=0; j<num_emiss; ++j)
      {
        ...
        {
          psum += emiss_probs.at(j,i);
          y.at(j,i) -= emiss_probs.at(j,i);
        }
      }
    }
    y_in = y; // input is now residual
  ...
  arma::mat reverse_differentiate (arma::mat& d_states, const arma::mat& gradient) 
  {
    // calculate (store) Gaussian loglikelihood of epoch
    // returns gradient of rate matrix wrt Gaussian loglikelihood
    // updates gradient of state vectors wrt Gaussian loglikelihood
    ...
    // "gradient" is Q (y - m)
    // "y" is (y - m)
    loglikelihood = -0.5 * arma::accu(gradient % y); //member
...
struct decoder_gaussian
{
  transition_rate_matrix trans_mat;

  decoder_gaussian (const unsigned num_pop, const unsigned num_ind, const bool approx) : trans_mat(num_pop,num_ind,approx) {}
  ...
  arma::cube coalescence_rates (arma::mat n, const arma::cube& y)
  {
    ...
    for (unsigned t=0; t<y.n_slices; ++t)
    {
      ...
      {
        rates.slice(t).col(i) = arma::log(y.slice(t).col(i)) - log(n.at(i));
        n.at(i) -= arma::accu(y.slice(t).col(i));
      }
    }
    return rates;
  }
  ...
  Rcpp::List random_walk_prior (const arma::cube& migr_mat, const arma::mat& penalty, const unsigned order)
  {
    if(!(migr_mat.n_rows == migr_mat.n_cols && arma::all(arma::vectorise(migr_mat) >= 0.))) Rcpp::stop("invalid migr_mat");
    if(!(arma::all(arma::vectorise(penalty) >= 0.) && penalty.n_rows == migr_mat.n_rows && penalty.n_cols == migr_mat.n_cols)) Rcpp::stop("invalid penalty");

    unsigned T = migr_mat.n_slices;
    arma::cube d_migr_mat (arma::size(migr_mat), arma::fill::zeros);
    ...
    for (unsigned i=0; i<migr_mat.n_rows; ++i)
    {
      for (unsigned j=0; j<migr_mat.n_cols; ++j)
      {
        arma::vec diff_ij = diff * arma::log10(arma::vectorise(migr_mat.tube(i,j)));
        lp += arma::accu(-0.5 * arma::pow(diff_ij, 2) * std::pow(penalty.at(i,j), 2));
  ...
  Rcpp::List deviance (const arma::mat& states, const arma::sp_mat& prec, const arma::cube& y, const arma::umat& remap, const arma::cube& migr_mat, const arma::vec& duration)
  {
    unsigned T = duration.n_elem;
    ...
    std::vector<coalescent_epoch_gaussian> epochs;
    epochs.reserve(T);

    arma::mat states_copy = states;
    arma::cube emiss_probs (trans_mat.num_emiss, trans_mat.num_start, T);
    arma::cube resid = y;

    for (unsigned t=0; t<T; ++t)
    {
      epochs.emplace_back(states_copy, resid.slice(t), remap.col(t), migr_mat.slice(t), duration.at(t), &trans_mat);
      emiss_probs.slice(t) = epochs[t].emiss_probs;
    }

    arma::vec weighted_resid = prec * arma::vectorise(resid);
    arma::cube emission_gradient (weighted_resid.memptr(), resid.n_rows, resid.n_cols, resid.n_slices);
    arma::cube gradient (arma::size(migr_mat));
    states_copy.zeros();
    ...
    for (int t=T-1; t>=0; --t)
    {
      gradient.slice(t) = epochs[t].reverse_differentiate(states_copy, emission_gradient.slice(t));
      lp += epochs[t].loglikelihood;
    }
    ...
    return Rcpp::List::create(
        Rcpp::_["deviance"] = -2. * lp,
        Rcpp::_["gradient"] = -2. * gradient,
        Rcpp::_["states"] = -2. * states_copy, // gradient wrt initial states
        Rcpp::_["emission"] = emiss_probs + resid,
        Rcpp::_["predicted"] = emiss_probs
        );
  }
...
RCPP_MODULE(particle) {
  ...
  class_<decoder_gaussian>("decoder_gaussian")
    ...
    .method("emission_states", &decoder_gaussian::emission_states)
    .method("transitory_states", &decoder_gaussian::transitory_states)
    .method("coalescent_states", &decoder_gaussian::coalescent_states)
    .method("coalescence_rates", &decoder_gaussian::coalescence_rates)
    .method("map_coalescent_states_to_emission_states", &decoder_gaussian::map_coalescent_states_to_emission_states)
    .method("map_emission_classes_to_transitory_states", &decoder_gaussian::map_emission_classes_to_transitory_states)
    .method("transition_probabilities", &decoder_gaussian::transition_probabilities)
    .method("transition_rates", &decoder_gaussian::transition_rates)
    .method("random_walk_prior", &decoder_gaussian::random_walk_prior)
    .method("migration_function", &decoder_gaussian::migration_function)
    .method("migration_operator", &decoder_gaussian::migration_operator)
    .method("deviance", &decoder_gaussian::deviance)
    .method("initial_states", &decoder_gaussian::initial_states)
    ;
